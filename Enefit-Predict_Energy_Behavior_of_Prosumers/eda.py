...
def prepare_geoopt(data_dir:str,forecast_weather:pd.DataFrame)->pd.DataFrame:
    ...
    new_df["county"] = new_df["county"].astype(object) # co compatibility with other data

    print(f"Saving dataframe with {new_df.shape} shape. "
          +"Size: {:.2f}".format(new_df.memory_usage().sum() / 1024 ** 2))
...
class FeatureProcessorClass():
    ...
    def __call__(self,
        ...
        print(f"Location: {location.keys()}")
        ...
        print('4',df[self.weather_join].dtypes)
        print('4',historical_weather[self.weather_join].dtypes)
    ...
    def create_historical_weather_features(self, historical_weather, location):
        ...
        # print(len(historical_weather["latitude"]), historical_weather[self.weather_join].dtypes)
        # Add county
        ...
        print('1',len(historical_weather["latitude"]), historical_weather[self.weather_join].dtypes)

        # Group by & calculate aggregate stats
        agg_columns = [col for col in historical_weather.columns if col not in self.weather_join]#+self.lat_lon_columns]
        agg_dict = {agg_col: self.agg_stats for agg_col in agg_columns}
        historical_weather = historical_weather.groupby(self.weather_join+self.lat_lon_columns).agg(agg_dict).reset_index()
        # Flatten the multi column aggregates
        historical_weather = self.flatten_multi_index_columns(historical_weather)

        print('2',len(historical_weather["latitude"]), historical_weather[self.weather_join].dtypes)
        ...
        print('3',len(historical_weather["latitude"]), historical_weather[self.weather_join].dtypes)
...
if __name__ == '__main__':
    ...
    print("\t> Data is collated successfully. Size: {:.2f}".format(data.memory_usage().sum() / 1024 ** 2))
    ...
    print("\t> Lagged data is added successfully. New size {:.2f}".format(df.memory_usage().sum() / 1024 ** 2))

    # save dataset
    if IS_LOCAL:
        print("\t> Saving dataframe. Size {:.2f}".format(df.memory_usage().sum() / 1024 ** 2))
        df.to_csv(data_dir + "collated_train.csv", index=False)
        print("\t> Resulting dataframe is saved successfully")
    ...
    df = load_reduce_memory_dataset(data_dir="./data/")
    print("\t> Dataset size reduced to {}".format(df.memory_usage().sum() / 1024 ** 2))
